...
export const DefaultPagination: React.FunctionComponent<DefaultPaginationProps> = ({
  currentPage,
  maxPage,
  onPageRequest,
}) => (
  <Pagination listClassName="justify-content-center" size="md">
    ...
    {currentPage > 0
      ? _.range(
          currentPage - 1,
          (currentPage - PAGINATION_ITEMS_COUNT_LEFT >= 0
            ? currentPage - PAGINATION_ITEMS_COUNT_LEFT
            : 0) - 1,
          -1
        )
          .reverse()
          .map(page => (
            <PaginationItem key={page}>
              <PaginationLink onClick={() => onPageRequest(page)}>
                {page + 1}
              </PaginationLink>
            </PaginationItem>
          ))
    ...
    {currentPage + 1 <= maxPage
      ? _.range(
          currentPage + 1,
          (currentPage + PAGINATION_ITEMS_COUNT_RIGHT <= maxPage
            ? currentPage + PAGINATION_ITEMS_COUNT_RIGHT
            : maxPage) + 1
        ).map(page => (
          <PaginationItem key={page}>
            <PaginationLink onClick={() => onPageRequest(page)}>
