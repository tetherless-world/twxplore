...
from geo_cli.model.uls_antenna import UlsAntenna
from geo_cli.model.uls_entity import UlsEntity
from geo_cli.model.uls_location import UlsLocation


class _UlsTransformer:
    def _transform_antennae(self, zip_file: ZipFile) -> Generator[UlsAntenna, None, None]:
        with zip_file.open("AN.dat") as an_file:
            for line in TextIOWrapper(an_file, "utf-8"):
                columns = line.split("|")
                # Antenna
    ...
    def _transform_entities(self, zip_file: ZipFile) -> Generator[UlsEntity, None, None]:
    ...
    def _transform_locations(self, zip_file: ZipFile) -> Generator[UlsLocation, None, None]:
        with zip_file.open("LO.dat") as lo_file:
            for line in TextIOWrapper(lo_file, "utf-8"):
                columns = line.split("|")
                # Location
                # Position Data Element Definition
                # [LO]
                ...
                if not name:
                    continue
                street_address = columns[11].strip()
                if not street_address:
                    continue
                city = columns[12].strip()
                if not city:
                    continue
                state = columns[14].upper().strip()
                if not state:
                    continue
                # elif state != "NY":
                #     continue
